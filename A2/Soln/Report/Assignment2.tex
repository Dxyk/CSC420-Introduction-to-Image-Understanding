...
\begin{document}
    ...
    \begin{homeworkProblem}
    	...
    	\begin{enumerate}
    		\item
            The outputs for Harris corner detection is shown in Figure \ref{fig:2.1.1}.
            ...
            The output for Brown Corner detection is shown in Figure \ref{fig:2.1.2}
            \begin{figure}[!htb]
            ...
            By rotating the detected corners back, we get the following Figure \ref{fig:2.2.4}.
            
            To prove that the detected points are also rotated by the same angle, we count the number of detected corners in the rotated image ($num\_total\_rotated$). Then we also count the number of corners in the original image corresponding to the coordinates of the corners in the rotated image ($num\_match$). Then we take the fraction of those two $\dfrac{num\_match}{num\_total\_rotated}$. The result was $99\%$ which is sufficient to prove that the detected corners are the same.
            ...
            \newpage
            \item 
            The interest points detected using different layers of Laplacian of Gaussian are shown bellow
            ...
            The key point detection algorithm is similar to that of SIFT's, but the blob detectors are different.
            ...
            1. fix a reproducible orientation based on information from a circular region around the interest point.
    ...
    \begin{homeworkProblem}
        ...
        \begin{enumerate}
            ...
            By testing with different threshold values, the following graph was produced. This is reasonable because as we loosen our restrictions, more and more points will be matched between the two images. (we accept more less distinctive points)
            ...
            From the results, we conclude that all the detected points are plausible. However for L3, there are more points that are too close to each other, and for L2, there are points detected that are less corner-like. Thus L1 is better in this case because it contains more corners
            ...
            This is because adding noise will change the pixel's descriptor because the magnitude and the orientation of the gradient near each pixel will change due to the noise.
